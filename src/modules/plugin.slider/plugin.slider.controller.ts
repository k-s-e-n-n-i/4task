...
export class Controller {
  ...
  changeConfigInputMin(val: number): void {
    ...
    let inputElemMinStart: HTMLInputElement = this.thisSlider.querySelector(
  ...
  changeConfigInputMax(val: number): void {
    ...
    let inputElemMaxStart: HTMLInputElement = this.thisSlider.querySelector(
  ...
  applyConfig(): void {
    let thisClick: HTMLInputElement = this.model.checkboxSettings,
      contr: Controller = this;
    ...
    function clear(thisSlider: HTMLElement, id: number) {
      ...
      const rangeLeftElem: HTMLElement = thisSlider.querySelector(
        '.range-slider#idSlider' + id + ' .range-slider__left'
      );
      rangeLeftElem.style.opacity = '1';
      const rangeElem: HTMLElement = thisSlider.querySelector(
  ...
  writeDataInConfig(): void {
    ...
    const configRadioElemType: HTMLInputElement = this.thisSlider.querySelector(
      `.radiogroup__input[name=rbGroopType${id}]#rbrbGroopType${id}${id}${typeID}`
    );
    configRadioElemType.checked = true;

    const configRadioElemOrientation: HTMLInputElement = this.thisSlider.querySelector(
    ...
    const configRadioElemValue: HTMLInputElement = this.thisSlider.querySelector(
      `.radiogroup__input[name=rbGroopValue${id}]#rbrbGroopValue${id}${id}${valueID}`
    );
    configRadioElemValue.checked = true;

    const configRadioElemScale: HTMLInputElement = this.thisSlider.querySelector(
